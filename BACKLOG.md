...
| 2 | Execution engine and durability | self-critique | reviews/phase-2.md |
...
Exit criterion: the kill-and-resume test passes under Postgres, and two concurrent inbound messages are processed in order with no lost state. Met on 2026-09-05: 450 tests green, including sixteen kill-and-resume combinations that each reproduce a byte-equal trace, frame stack, status and transcript, and `test_two_processes_send_at_once_and_the_turns_run_in_order`, which reads the ordering out of the customer's transcript rather than out of timing. `tests/stepper.py` is deleted: the executor replaced it, and `tests/test_engine_graph.py` carries the phase-1 exit criterion's assertions against the real engine.
