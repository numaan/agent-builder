...
| 3 | LLM layer and prompted nodes | in-review | reviews/phase-3.md |
...
Exit criterion: `packs/acme_billing` has a `root.yaml` with a classify node and a `small_talk` path, and a scripted conversation runs through it with the fake provider. Met on 2026-09-06: 590 tests green (one deselected - the `live` group), `tests/test_golden_conversation.py` replays a two-turn conversation through `root.yaml` against `FakeProvider`, which refuses anything whose request fingerprint it has not recorded, and a fourth test re-records the scenario and requires the committed cassette back byte for byte. The sample pack is no longer "empty but well-formed": it validates *with* graphs, and the three phase-0 tests that pinned that wording now assert the CLI contract they were protecting.
